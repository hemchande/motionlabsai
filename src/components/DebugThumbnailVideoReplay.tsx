...
import { Play, Pause, RotateCcw, Volume2, VolumeX, Maximize2, Clock, User, Calendar, Award, Shield, Loader2, AlertTriangle, CheckCircle } from "lucide-react"
...
interface DebugThumbnailVideoReplayProps {
...
export default function DebugThumbnailVideoReplay({
  session,
  onViewFullVideo,
  onViewAnalytics,
  compact = false,
  ...
  replayDuration = 4,
  debug = true
}: DebugThumbnailVideoReplayProps) {
  ...
  useEffect(() => {
    if (debug) {
      console.log('DebugThumbnailVideoReplay - Session:', session)
      console.log('DebugThumbnailVideoReplay - Video URL:', session.processedVideoUrl)
    }
  }, [session, debug])
  ...
  }, [autoPlay, replayDuration, isReplaying, debug])
              ...
              <div className="text-red-400 text-center">
                ...
                {debug && (
                  <div className="text-xs text-slate-600 mt-2 space-y-1">
                    <p>URL: {session.processedVideoUrl}</p>
                    {debugInfo.error && <p>Error: {debugInfo.error}</p>}
                    {debugInfo.networkState !== undefined && <p>Network State: {debugInfo.networkState}</p>}
                    {debugInfo.readyState !== undefined && <p>Ready State: {debugInfo.readyState}</p>}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-3 space-y-2">
            ...
            {debug && (
              <div className="text-xs text-slate-500 space-y-1 pt-2 border-t border-slate-700">
                <p>Video URL: {session.processedVideoUrl}</p>
                {debugInfo.duration && <p>Duration: {debugInfo.duration.toFixed(2)}s</p>}
                {debugInfo.videoWidth && <p>Size: {debugInfo.videoWidth}x{debugInfo.videoHeight}</p>}
                {debugInfo.readyState !== undefined && <p>Ready State: {debugInfo.readyState}</p>}
              </div>
            )}
